...
namespace Catalog.API.Repositories
{
    public class ProductRepository : IProductRepository
    {
        ...
        {
            return await _context.Products.Find(product => product.Id == id)
                                          .FirstOrDefaultAsync();
        }

        public async Task<IEnumerable<Product>> GetProducts()
        {
            return await _context.Products.Find(product => true)
                                          .ToListAsync();
        }
        ...
        {
            ReplaceOneResult updateResult = await _context.Products.ReplaceOneAsync(filter: current => current.Id == product.Id, replacement: product);
